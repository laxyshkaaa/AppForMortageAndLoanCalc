...
namespace КУРСОВАЯ
{
    public partial class HistoryForm : Form
    {
        ...
        private async void UpdateKeyRate_Click(object sender, EventArgs e)
        {
            KeyRateUpdater keyRateUpdater = new KeyRateUpdater();
            double keyRate = await keyRateUpdater.GetKeyRate();

            if (keyRate > 0)
            {
                lblKeyRate.Text = $"Текущий курс доллара к рублю: {keyRate}";
            }
            else
            {
                MessageBox.Show("Не удалось обновить курс доллара. Проверьте подключение к интернету.");
            }
        }
    }
}
